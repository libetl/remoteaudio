...
public class Connection {
    ...
    public synchronized boolean authenticateWithDSA (String user, String pem,
        ...
        this.authenticated = this.am.authenticatePublicKey (user,
                pem.toCharArray (), password, this.getOrCreateSecureRND ());
    ...
    public synchronized boolean authenticateWithKeyboardInteractive (
        ...
        this.authenticated = this.am.authenticateInteractive (user, submethods,
                cb);
     ...
     *            "-----BEGIN RSA PRIVATE KEY-----" tag). The char array may
     *            contain linebreaks/linefeeds.
     * @param password
    ...
    public synchronized boolean authenticateWithPublicKey (String user,
        ...
        this.authenticated = this.am.authenticatePublicKey (user,
                pemPrivateKey, password, this.getOrCreateSecureRND ());
    ...
    public synchronized boolean authenticateWithPublicKey (String user,
        ...
        return this.authenticateWithPublicKey (user, cw.toCharArray (),
                password);
    }
    ...
    public synchronized ConnectionInfo connect (ServerHostKeyVerifier verifier,
        ...
        if (this.tm != null) {
            throw new IOException ("Connection to " + this.hostname
        ...
        try {
            ...
            try {
                this.tm.initialize (this.cryptoWishList, verifier,
                        this.dhgexpara, connectTimeout,
                        this.getOrCreateSecureRND (), this.proxyData);
            ...
            this.close (new Throwable ("There was a problem during connect."),
                    false);

            synchronized (state) {
                /*
                 * Show a clean exception, not something like
            ...
            throw (IOException) new IOException (
                    "There was a problem while connecting to " + this.hostname
                            + ":" + this.port).initCause (e1);
        }
    }
    ...
    public synchronized LocalPortForwarder createLocalPortForwarder (
        ...
        return new LocalPortForwarder (this.cm, local_port, host_to_connect,
                port_to_connect);
    }
    ...
    public synchronized LocalStreamForwarder createLocalStreamForwarder (
        ...
        return new LocalStreamForwarder (this.cm, host_to_connect,
                port_to_connect);
    }
     ...
     * @throws IOException
     */
    public synchronized boolean isAuthMethodAvailable (String user,
    ...
    private String [] removeDuplicates (String [] list) {
        ...
        for (String element : list) {
            ...
            for (int j = 0; j < count; j++ ) {
                if (element == null && list2 [j] == null || element != null
                        && element.equals (list2 [j])) {
                    duplicate = true;
                    break;
                }
            }
    ...
    public synchronized void requestRemotePortForwarding (String bindAddress,
        ...
        this.cm.requestGlobalForward (bindAddress, bindPort, targetAddress,
                targetPort);
    }
