...
public class KnownHosts {
    ...
    private int checkKey (String remoteHostname, Object remoteKey) {
        int result = KnownHosts.HOSTKEY_IS_NEW;

        synchronized (this.publicKeys) {
            Iterator<KnownHostsEntry> i = this.publicKeys.iterator ();

            while (i.hasNext ()) {
                KnownHostsEntry ke = i.next ();

                if (this.hostnameMatches (ke.patterns, remoteHostname) == false) {
                    continue;
                }
                ...
                if (res == true) {
                    return KnownHosts.HOSTKEY_IS_OK;
                }

                result = KnownHosts.HOSTKEY_HAS_CHANGED;
            }
        }
        return result;
    }
    ...
    private Vector<Object> getAllKeys (String hostname) {
        ...
        synchronized (this.publicKeys) {
            ...
            while (i.hasNext ()) {
                ...
                if (this.hostnameMatches (ke.patterns, hostname) == false) {
                    continue;
                }
    ...
    public String [] getPreferredServerHostkeyAlgorithmOrder (String hostname) {
        ...
        for (InetAddress ipAdresse : ipAdresses) {
            algos = this.recommendHostkeyAlgorithms (ipAdresse
                    .getHostAddress ());
    ...
    private final boolean hostnameMatches (String [] hostpatterns,
            String hostname) {
        ...
        for (String hostpattern : hostpatterns) {
            ...
            if (pattern.charAt (0) == '|') {
                ...
                if (pattern.indexOf ('?') != -1 || pattern.indexOf ('*') != -1) {
                    if (this.pseudoRegex (pattern.toCharArray (), 0,
    ...
    private final boolean pseudoRegex (char [] pattern, int i, char [] match,
            int j) {
        ...
        while (true) {
            ...
            if (pattern [i] == '*') {
                ...
                if (pattern [i] != '*' && pattern [i] != '?') {
                    while (true) {
                        if (pattern [i] == match [j]
                                && this.pseudoRegex (pattern, i + 1, match,
                                        j + 1)) {
                            return true;
                        }
                        j++ ;
                ...
                while (true) {
                    if (this.pseudoRegex (pattern, i, match, j)) {
                        return true;
                    }
                    j++ ;
                    if (match.length == j) {
    ...
    private String [] recommendHostkeyAlgorithms (String hostname) {
        ...
        Vector<Object> keys = this.getAllKeys (hostname);
    ...
    public int verifyHostkey (String hostname, String serverHostKeyAlgorithm,
        ...
        for (InetAddress ipAdresse : ipAdresses) {
            int newresult = this.checkKey (ipAdresse.getHostAddress (),
                    remoteKey);

            if (newresult == KnownHosts.HOSTKEY_IS_OK) {
                return newresult;
            }

            if (newresult == KnownHosts.HOSTKEY_HAS_CHANGED) {
                result = KnownHosts.HOSTKEY_HAS_CHANGED;
            }
        }
