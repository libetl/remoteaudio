...
public class ChannelManager implements MessageHandler {
    ...
    public X11ServerData checkX11Cookie (String hexFakeCookie) {
        synchronized (this.x11_magic_cookies) {
            if (hexFakeCookie != null) {
                return (X11ServerData) this.x11_magic_cookies
                        .get (hexFakeCookie);
            }
        }
        return null;
    ...
    public void closeAllChannels () {
        ...
        synchronized (this.channels) {
            channel_copy = (Vector<Channel>) this.channels.clone ();
        }
    ...
    public void closeChannel (Channel c, String reason, boolean force)
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (50, "Sent SSH_MSG_CHANNEL_CLOSE (channel "
                    + c.localID + ")");
        }
    }
    ...
    public int getChannelData (Channel c, boolean extended, byte [] target,
        ...
        if (increment > 0) {
            if (ChannelManager.log.isEnabled ()) {
                ChannelManager.log.log (80,
                        "Sending SSH_MSG_CHANNEL_WINDOW_ADJUST (channel "
                                + localID + ", " + increment + ")");
            }
        ...
        if (msg == null) {
            ...
            synchronized (this.listenerThreads) {
                for (int i = 0; i < this.listenerThreads.size (); i++ ) {
                    IChannelWorkerThread lat = (IChannelWorkerThread) this.listenerThreads
                            .elementAt (i);
                    lat.stopWorking ();
                }
            ...
            synchronized (this.channels) {
                ...
                for (int i = 0; i < this.channels.size (); i++ ) {
                    ...
                    synchronized (c) {
                        ...
                        c.closeMessageRecv = true; /*
                                                    * You never know, perhaps we
                                                    * are waiting for a pending
                                                    * close message from the
                                                    * server...
                                                    */
                        c.notifyAll ();
                    }
                ...
                this.channels.notifyAll (); /* Notify global response waiters */
                return;
            }
        }
    ...
    public void msgChannelClose (byte [] msg, int msglen) throws IOException {
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (50, "Got SSH_MSG_CHANNEL_CLOSE (channel "
                    + id + ")");
        }
    }
    ...
    public void msgChannelData (byte [] msg, int msglen) throws IOException {
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (80, "Got SSH_MSG_CHANNEL_DATA (channel "
                    + id + ", " + len + ")");
        }
    ...
    public void msgChannelEOF (byte [] msg, int msglen) throws IOException {
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (50, "Got SSH_MSG_CHANNEL_EOF (channel "
                    + id + ")");
        }
    }
    ...
    public void msgChannelExtendedData (byte [] msg, int msglen)
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (80,
                    "Got SSH_MSG_CHANNEL_EXTENDED_DATA (channel " + id + ", "
                            + len + ")");
        }
    ...
    public void msgChannelFailure (byte [] msg, int msglen) throws IOException {
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (50, "Got SSH_MSG_CHANNEL_FAILURE (channel "
                    + id + ")");
        }
    }
    ...
    public void msgChannelOpen (byte [] msg, int msglen) throws IOException {
        ...
        if ("x11".equals (channelType)) {
            synchronized (this.x11_magic_cookies) {
                ...
                if (this.x11_magic_cookies.size () == 0) {
                    ...
                    if (ChannelManager.log.isEnabled ()) {
                        ChannelManager.log.log (20,
                                "Unexpected X11 request, denying it!");
                    }
        ...
        if ("forwarded-tcpip".equals (channelType)) {
            ...
            synchronized (this.remoteForwardings) {
                rfd = (RemoteForwardingData) this.remoteForwardings
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (20,
                    "The peer tried to open an unsupported channel type ("
                            + channelType + ")");
        }
    }
    ...
    public void msgChannelOpenFailure (byte [] msg, int msglen)
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (50,
                    "Got SSH_MSG_CHANNEL_OPEN_FAILURE (channel " + id + ")");
        }
    }
    ...
    public void msgChannelRequest (byte [] msg, int msglen) throws IOException {
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (80, "Got SSH_MSG_CHANNEL_REQUEST (channel "
                    + id + ", '" + type + "')");
        }
        ...
        if (type.equals ("exit-status")) {
            ...
            if (ChannelManager.log.isEnabled ()) {
                ChannelManager.log.log (50, "Got EXIT STATUS (channel " + id
                        + ", status " + exit_status + ")");
            }
        ...
        if (type.equals ("exit-signal")) {
            ...
            if (ChannelManager.log.isEnabled ()) {
                ChannelManager.log.log (50, "Got EXIT SIGNAL (channel " + id
                        + ", signal " + signame + ")");
            }
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (50, "Channel request '" + type
    ...
    public void msgChannelSuccess (byte [] msg, int msglen) throws IOException {
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (80, "Got SSH_MSG_CHANNEL_SUCCESS (channel "
                    + id + ")");
        }
    }
    ...
    public void msgChannelWindowAdjust (byte [] msg, int msglen)
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (80,
                    "Got SSH_MSG_CHANNEL_WINDOW_ADJUST (channel " + id + ", "
                            + windowChange + ")");
        }
    }
    ...
    public void msgGlobalRequest (byte [] msg, int msglen) throws IOException {
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (80, "Got SSH_MSG_GLOBAL_REQUEST ("
                    + requestName + ")");
        }
    }
    ...
    public Channel openSessionChannel () throws IOException {
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (50,
                    "Sending SSH_MSG_CHANNEL_OPEN (Channel " + c.localID + ")");
        }
    ...
    public void registerThread (IChannelWorkerThread thr) throws IOException {
        synchronized (this.listenerThreads) {
            if (this.listenerThreadsAllowed == false) {
    ...
    public void requestExecCommand (Channel c, String cmd) throws IOException {
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (50, "Executing command (channel "
                    + c.localID + ", '" + cmd + "')");
        }
    ...
    public int requestGlobalForward (String bindAddress, int bindPort,
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (50, "Requesting a remote forwarding ('"
                    + bindAddress + "', " + bindPort + ")");
        }
    ...
    public void requestSubSystem (Channel c, String subSystemName)
        ...
        try {
            ...
            throw (IOException) new IOException (
                    "The subsystem request failed.").initCause (e);
        }
    }
    ...
    public void requestX11 (Channel c, boolean singleConnection,
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (50, "Requesting X11 forwarding (Channel "
                    + c.localID + "/" + c.remoteID + ")");
        }
        ...
        while (len > 0) {
            ...
            synchronized (c) {
                while (true) {
                    ...
                    if (c.state != Channel.STATE_OPEN) {
                        throw new IOException (
                                "SSH channel in strange state. (" + c.state
                                        + ")");
                    }
                ...
                int estimatedMaxDataLen = c.remoteMaxPacketSize
                        - (this.tm.getPacketOverheadEstimate () + 9);
    ...
    public void sendEOF (Channel c) throws IOException {
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (50, "Sent EOF (Channel " + c.localID + "/"
                    + c.remoteID + ")");
        }
    }
    ...
    public void unRegisterX11Cookie (String hexFakeCookie, boolean killChannels) {
        ...
        if (ChannelManager.log.isEnabled ()) {
            ChannelManager.log.log (50,
                    "Closing all X11 channels for the given fake cookie");
        }
    ...
    private final void waitForGlobalSuccessOrFailure () throws IOException {
        synchronized (this.channels) {
            while (this.globalSuccessCounter == 0
