...
public class RemoteAcceptThread extends Thread {
    ...
    public RemoteAcceptThread (Channel c, String remoteConnectedAddress,
        ...
        if (RemoteAcceptThread.log.isEnabled ()) {
            RemoteAcceptThread.log.log (20, "RemoteAcceptThread: "
                    + remoteConnectedAddress + "/" + remoteConnectedPort
                    + ", R: " + remoteOriginatorAddress + "/"
    ...
    public void run () {
        try {
            ...
            StreamForwarder r2l = new StreamForwarder (this.c, null, null,
                    this.c.getStdoutStream (), this.s.getOutputStream (),
                    "RemoteToLocal");
            StreamForwarder l2r = new StreamForwarder (this.c, null, null,
                    this.s.getInputStream (), this.c.getStdinStream (),
                    "LocalToRemote");
            ...
            this.c.cm.closeChannel (this.c, "EOF on both streams reached.",
                    true);
            this.s.close ();
        } catch (IOException e) {
            RemoteAcceptThread.log.log (50,
                    "IOException in proxy code: " + e.getMessage ());

            try {
                this.c.cm.closeChannel (this.c, "IOException in proxy code ("
                        + e.getMessage () + ")", true);
