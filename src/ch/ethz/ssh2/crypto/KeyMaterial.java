...
public class KeyMaterial {
    ...
    public static KeyMaterial create (String hashType, byte [] H, BigInteger K,
        ...
        km.initial_iv_client_to_server = KeyMaterial.calculateKey (sh, K, H,
                (byte) 'A', SessionID, blockSizeCS);

        km.initial_iv_server_to_client = KeyMaterial.calculateKey (sh, K, H,
                (byte) 'B', SessionID, blockSizeSC);

        km.enc_key_client_to_server = KeyMaterial.calculateKey (sh, K, H,
                (byte) 'C', SessionID, keyLengthCS);

        km.enc_key_server_to_client = KeyMaterial.calculateKey (sh, K, H,
                (byte) 'D', SessionID, keyLengthSC);

        km.integrity_key_client_to_server = KeyMaterial.calculateKey (sh, K, H,
                (byte) 'E', SessionID, macLengthCS);

        km.integrity_key_server_to_client = KeyMaterial.calculateKey (sh, K, H,
