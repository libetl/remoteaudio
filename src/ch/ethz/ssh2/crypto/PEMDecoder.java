...
public class PEMDecoder {
    ...
    public static Object decode (char [] pem, String password)
        ...
        if (PEMDecoder.isPEMEncrypted (ps)) {
            ...
            PEMDecoder.decryptPEM (ps, password.getBytes ());
        }

        if (ps.pemType == PEMDecoder.PEM_DSA_PRIVATE_KEY) {
        ...
        if (ps.pemType == PEMDecoder.PEM_RSA_PRIVATE_KEY) {
            SimpleDERReader dr = new SimpleDERReader (ps.data);
    ...
    private static final void decryptPEM (PEMStructure ps, byte [] pw)
        ...
        byte [] salt = PEMDecoder.hexToByteArray (ps.dekInfo [1]);

        BlockCipher bc = null;

        if (algo.equals ("DES-EDE3-CBC")) {
            DESede des3 = new DESede ();
            des3.init (false, PEMDecoder.generateKeyFromPasswordSaltWithMD5 (
                    pw, salt, 24));
            bc = new CBCMode (des3, salt, false);
        } else if (algo.equals ("DES-CBC")) {
            DES des = new DES ();
            des.init (false,
                    PEMDecoder.generateKeyFromPasswordSaltWithMD5 (pw, salt, 8));
            bc = new CBCMode (des, salt, false);
        } else if (algo.equals ("AES-128-CBC")) {
            AES aes = new AES ();
            aes.init (false, PEMDecoder.generateKeyFromPasswordSaltWithMD5 (pw,
                    salt, 16));
        ...
        dz = PEMDecoder.removePadding (dz, bc.getBlockSize ());
    ...
    private static byte [] generateKeyFromPasswordSaltWithMD5 (
            byte [] password, byte [] salt, int keyLen) throws IOException {
        if (salt.length < 8) {
        ...
        while (true) {
            ...
            md5.update (salt, 0, 8); // ARGH we only use the first 8 bytes of
            // the salt in this step.
    ...
    private static final PEMStructure parsePEM (char [] pem) throws IOException {
        ...
        while (true) {
            ...
            if (line.startsWith ("-----BEGIN DSA PRIVATE KEY-----")) {
                ...
                ps.pemType = PEMDecoder.PEM_DSA_PRIVATE_KEY;
                break;
            }

            if (line.startsWith ("-----BEGIN RSA PRIVATE KEY-----")) {
                endLine = "-----END RSA PRIVATE KEY-----";
                ps.pemType = PEMDecoder.PEM_RSA_PRIVATE_KEY;
                break;
            }
        }
