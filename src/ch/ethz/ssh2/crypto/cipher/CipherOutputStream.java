...
public class CipherOutputStream {
    ...
    byte []      out_buffer     = new byte [this.BUFF_SIZE];
    ...
    private void internal_write (byte [] src, int off, int len)
            throws IOException {
        while (len > 0) {
            int space = this.BUFF_SIZE - this.out_buffer_pos;
            int copy = len > space ? space : len;

            System.arraycopy (src, off, this.out_buffer, this.out_buffer_pos,
                    copy);
            ...
            if (this.out_buffer_pos >= this.BUFF_SIZE) {
                this.bo.write (this.out_buffer, 0, this.BUFF_SIZE);
                this.out_buffer_pos = 0;
            }
        }
    }
    ...
    private void writeBlock () throws IOException {
        try {
            ...
            throw (IOException) new IOException (
                    "Error while decrypting block.").initCause (e);
        }
