...
public class RSASHA1Verify {
    ...
    public static RSASignature decodeSSHRSASignature (byte [] sig)
        ...
        if (RSASHA1Verify.log.isEnabled ()) {
            RSASHA1Verify.log.log (80,
                    "Decoding ssh-rsa signature string (length: " + s.length
                            + ")");
        }
    ...
    public static boolean verifySignature (byte [] message, RSASignature ds,
        ...
        if (n.compareTo (s) <= 0) {
            RSASHA1Verify.log
                    .log (20, "ssh-rsa signature: n.compareTo(s) <= 0");
            return false;
        }
        ...
        if (v [startpos] != 0x01) {
            RSASHA1Verify.log
                    .log (20, "ssh-rsa signature: v[startpos] != 0x01");
            return false;
        }
        ...
        while (true) {
            if (pos >= v.length) {
                RSASHA1Verify.log
                        .log (20, "ssh-rsa signature: pos >= v.length");
                return false;
            }
            if (v [pos] == 0x00) {
                break;
            ...
            if (v [pos] != (byte) 0xff) {
                RSASHA1Verify.log.log (20,
                        "ssh-rsa signature: v[pos] != (byte) 0xff");
                return false;
            }
            pos++ ;
        }
        ...
        if (dr.available () != 0) {
            RSASHA1Verify.log
                    .log (20, "ssh-rsa signature: dr.available() != 0");
            return false;
        }
        ...
        if (dr.available () != 0) {
            RSASHA1Verify.log.log (20,
                    "ssh-rsa signature: dr.available() != 0 (II)");
            return false;
        }
        ...
        for (int i = 0; i < sha_message.length; i++ ) {
            if (sha_message [i] != digest [i]) {
                RSASHA1Verify.log.log (20,
                        "ssh-rsa signature: sha_message[i] != digest[i]");
                return false;
            }
        }
